...
import 'package:skilla/model/refresh_token.dart';
...
class CustomInterceptors extends InterceptorsWrapper {
  ...
  Future onError(DioError err) async {
    ...
    if (err.response?.statusCode == 401) {
      ...
      if (UserAuth.auth != null) {
        refreshToken = UserAuth.auth.refreshToken;
      } else {
      ...
      Map<String, dynamic> body = RefreshToken(refreshToken).toJson();

      Dio dio = Dio();
      String url = "${kBaseURL}auth/${kApiVersion}/session/refresh";

      RequestOptions options = RequestOptions(
          headers: await _getDefaultHeaders(), data: body, method: "POST");

      if (kDebugMode) {
        _printRequest(options, method: "POST", url: url);
      }

      var refreshTokenRequest =
          await dio.post(url, options: options).timeout(Duration(seconds: 60));

      if (kDebugMode) {
        _printResponse(refreshTokenRequest);
      }

      if (refreshTokenRequest.statusCode >= 200 &&
          refreshTokenRequest.statusCode <= 299) {
        var auth = Auth.fromJson(refreshTokenRequest.data['data']);
        auth.whenRefresh = Utils.whenRefreshToken(auth.accessTokenTTL);
        UserAuth.auth = auth;

        await AuthDAO().update(auth);
  ...
  Future<Map<String, String>> _getDefaultHeaders() async {
    ...
    if (UserAuth.auth != null) {
      headers['authorization'] = "Bearer ${UserAuth.auth.accessToken}";
    } else {
      var auth = await _getAuthorization();
      if (auth != null) {
